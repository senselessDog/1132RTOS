  ...
  void PeriodicTask1(void *pdata)
  {
      ...
      while(1) {
          ...
          while(((int)((OSTCBCur->period) - (OSTimeGet() - start))>0)&& OSTCBCur->compTime > 0 ) {
              //printf("[hello] Task %d: compTime = %d\n", (int)OSTCBCur->OSTCBPrio, (int)OSTCBCur->compTime);
              /* Do nothing, just consume CPU time */
        	  if (OSTCBCur->compTime == 9 && should_Run_R2){
        		  OSMutexPend(R2_Mutex,1000,&err);
        		  should_Run_R2=0;
        	  }else if (OSTCBCur->compTime == 6 && should_Run_R1){
        		  OSMutexPend(R1_Mutex,1000,&err);
        		  should_Run_R1=0;
        		  should_Run_R2=1;
        	  }else if(OSTCBCur->compTime == 3 && should_Run_R2){
        		  OSMutexPost(R1_Mutex);
        		  should_Run_R2=0;
        	  }
          }
...
//      OSTimeDly(4);
      ...
      while(1) {
          ...
          while(((int)((OSTCBCur->period) - (OSTimeGet() - start))>0)&& OSTCBCur->compTime > 0 ) {
              //printf("[hello] Task %d: compTime = %d\n", (int)OSTCBCur->OSTCBPrio, (int)OSTCBCur->compTime);
              /* Do nothing, just consume CPU time */
        	  if (OSTCBCur->compTime == 10 && should_Run_R1){
				  OSMutexPend(R1_Mutex,1000,&err);
				  should_Run_R1=0;
			  }else if (OSTCBCur->compTime == 4 && should_Run_R2){
				  OSMutexPend(R2_Mutex,1000,&err);
				  should_Run_R2=0;
				  should_Run_R1=1;
			  }else if(OSTCBCur->compTime == 2 && should_Run_R1){
				  OSMutexPost(R2_Mutex);
				  should_Run_R1=0;
			  }
          }
